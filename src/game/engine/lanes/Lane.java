...
public class Lane implements Comparable<Lane> {
	...
	public void setSelectedWeapons(ArrayList<Weapon> weapons) {
		this.weapons.clear(); // Clear existing weapons
		this.weapons.addAll(weapons); // Add new selected weapons
	}

	public List<WeaponRegistry> getPurchasedWeapons(WeaponFactory weaponFactory) {
		List<WeaponRegistry> purchasedWeapons = new ArrayList<>();
		for (Weapon weapon : this.getWeapons()) {
			WeaponRegistry registry = weaponFactory.getWeaponShop().values().stream()
					.filter(w -> w.buildWeapon().getClass().equals(weapon.getClass())).findFirst().orElse(null);
			if (registry != null) {
				purchasedWeapons.add(registry);
			}
		}
